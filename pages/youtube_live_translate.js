...
const showSourceSubtitles = document.getElementById("showSourceSubtitles");
...
const livePartialTranscript = document.getElementById("livePartialTranscript");
...
const translations = {
  "zh-Hant": {
    pageTitle: "Video Translate", uploadPageTitle: "影片上傳翻譯", transcribeOnly: "僅轉譯", urlLabel: "YouTube 網址", videoFileLabel: "選擇影片", dropVideos: "拖曳影片到這裡", dropVideosBrowse: "或點擊選擇影片", videoFileHelp: "一次最多選擇 10 支影片；檔案只用於本次轉譯，完成、取消或逾時後會自動清除。", invalidVideoFiles: "只支援影片檔案，已略過不支援的檔案。", videoFileLimit: "一次最多只能選擇 10 支影片。", videosSelected: "已選擇 {count} 支影片", uploadPrivacyLink: "查看隱私權政策", ignoreSubtitles: "忽略內建字幕", includeWordTimestamps: "逐字顯示原文", speakerDiarization: "講者辨識（Batch 測試）", speakerDiarizationHelp: "適合訪談與會議；啟用後會略過內建字幕，完成一批音訊後才顯示講者分段。", speakerLabel: "講者 {number}",
  ...
  en: {
    pageTitle: "Video Translate", uploadPageTitle: "Upload Video Translate", transcribeOnly: "Transcribe only", urlLabel: "YouTube URL", videoFileLabel: "Choose videos", dropVideos: "Drop videos here", dropVideosBrowse: "or click to choose videos", videoFileHelp: "Choose up to 10 videos. Files are used only for this batch and deleted after completion, cancellation, or expiration.", invalidVideoFiles: "Only video files are supported. Unsupported files were skipped.", videoFileLimit: "You can select up to 10 videos at a time.", videosSelected: "{count} videos selected", uploadPrivacyLink: "View Privacy Policy", ignoreSubtitles: "Ignore built-in subtitles", includeWordTimestamps: "Reveal source word by word", speakerDiarization: "Identify speakers (Batch test)", speakerDiarizationHelp: "For interviews and meetings. Built-in subtitles are skipped, and speaker segments appear after each audio batch completes.", speakerLabel: "Speaker {number}",
  ...
  ja: {
    pageTitle: "Video Translate", uploadPageTitle: "動画アップロード翻訳", transcribeOnly: "文字起こしのみ", urlLabel: "YouTube URL", videoFileLabel: "動画を選択", dropVideos: "動画をここにドロップ", dropVideosBrowse: "またはクリックして選択", videoFileHelp: "一度に最大 10 本まで選択できます。完了・キャンセル・期限切れ後に自動削除されます。", invalidVideoFiles: "動画ファイルのみ対応しています。未対応のファイルは除外しました。", videoFileLimit: "一度に選択できる動画は最大 10 本です。", videosSelected: "{count} 本の動画を選択済み", uploadPrivacyLink: "プライバシーポリシーを見る", ignoreSubtitles: "内蔵字幕を無視", includeWordTimestamps: "原文を単語ごとに表示", speakerDiarization: "話者識別（Batch テスト）", speakerDiarizationHelp: "インタビューや会議向けです。内蔵字幕を無視し、音声バッチの完了後に話者ごとの区間を表示します。", speakerLabel: "話者 {number}",
...
const speakerDisplayNumbers = new Map();
...
function recordTranslationUsage(usage) {
  if (selectedTranslationType === "private" || !usage || typeof usage !== "object") return;
  addTranslationUsage(translationUsageTotals, usage);
  updateTranslationProgress();
}

function withTranslationUsage(message) {
  if (selectedTranslationType === "private") return message;
  const summary = t("usageSummary", {
    tokens: Math.round(translationUsageTotals.countedInputTokens).toLocaleString(),
    usd: translationUsageTotals.estimatedCostUsd.toFixed(6),
    twd: translationUsageTotals.estimatedCostTwd.toFixed(4),
  });
...
function applyLanguage(languageKey) {
  ...
  for (const [segmentId, nodes] of segmentNodes.entries()) {
    const segment = sourceSegments.get(segmentId);
    if (nodes.speakerBadge && segment?.speakerId) {
      nodes.speakerBadge.textContent = speakerDisplayLabel(segment.speakerId);
    }
  }
  if (uploadMode) {
...
function configureSourceMode() {
  ...
  if (speakerDiarization.checked) ignoreSubtitles.checked = true;
...
function setSourceControlsDisabled(disabled) {
  if (uploadMode) {
    ...
    ignoreSubtitles.disabled = disabled || speakerDiarization.checked;
...
function speakerDisplayNumber(speakerId) {
  const normalized = String(speakerId || "").trim();
  if (!normalized) return null;
  const numericSuffix = normalized.match(/(\d+)$/);
  if (numericSuffix) return Number(numericSuffix[1]) + 1;
  if (!speakerDisplayNumbers.has(normalized)) {
    speakerDisplayNumbers.set(normalized, speakerDisplayNumbers.size + 1);
  }
  return speakerDisplayNumbers.get(normalized);
}

function speakerDisplayLabel(speakerId) {
  const number = speakerDisplayNumber(speakerId);
  return number == null ? "" : t("speakerLabel", { number });
}

function withSpeakerLabel(segment, text) {
  const label = speakerDisplayLabel(segment?.speakerId);
...
function updateVideoSubtitleOverlay(segment, currentTime = null) {
  ...
  if (segment && failureCode) {
    ...
    videoSubtitleSource.textContent = withSpeakerLabel(segment, sourceText);
    videoSubtitleSource.classList.toggle("d-none", !showSourceSubtitles.checked);
  ...
  if (!translatedText) {
    ...
    videoSubtitleSource.textContent = "";
    videoSubtitleSource.classList.add("d-none");
    videoSubtitleTranslation.textContent = withSpeakerLabel(segment, sourceText);
    videoSubtitleOverlay.classList.remove("waiting");
    videoSubtitleOverlay.classList.remove("error");
    return;
  }
  ...
  videoSubtitleSource.classList.toggle(
    "d-none",
    !showSourceSubtitles.checked || sourceText.trim() === translatedText.trim(),
  );
  videoSubtitleOverlay.classList.remove("waiting");
  videoSubtitleOverlay.classList.remove("error");
}
...
function currentTranslationIsSkipped() {
  return Boolean(
    ...
    && translationLanguagesMatch(requestedSourceLanguage, selectedTargetLanguage),
  );
}
...
function showLanguageConfirmation(data) {
  ...
  // Start from the detector's result so the next Together request receives
  // an explicit source language instead of its unreliable auto mode.
  confirmedSourceLanguage.value = requestedSourceLanguage
    ? (normalized || "")
    : (normalized || "auto");
...
function resetView() {
  ...
  currentCancelRequest = null;
  translationUsageTotals = emptyTranslationUsageTotals();
...
function updateTranslationProgress() {
  ...
  if (transcriptionDone) {
    ...
    translationDetail.textContent = withTranslationUsage(appendTranslationErrorCodes(t("translationPercentDone", {
      percent: rounded,
      failed,
    })));
  } else {
    ...
    translationDetail.textContent = withTranslationUsage(appendTranslationErrorCodes(detail));
  }
...
function renderSegment(segment) {
  ...
  speakerBadge.className = "badge text-bg-primary";
  speakerBadge.textContent = speakerDisplayLabel(segment.speakerId);
  speakerBadge.classList.toggle("d-none", !segment.speakerId);
  const groupBadge = document.createElement("span");
  groupBadge.className = "badge text-bg-secondary d-none";
  meta.append(time, speakerBadge, number, groupBadge);
...
function pendingSegmentsLikelyReady() {
  ...
  const hasSpeakerBoundary = pendingSegments.some((segment, index) => (
    index > 0 && segment.speakerId !== pendingSegments[index - 1].speakerId
  ));
...
async function translateBatch(batch) {
  ...
  const payload = {
    ...
    translation_type: selectedTranslationType,
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
    ...
    try {
      ...
      recordTranslationUsage(data?.usage);
...
function clearLivePartialTranscript() {
  livePartialText.textContent = "";
  livePartialTranscript.classList.add("d-none");
}

function showLivePartialTranscript(data) {
  if (jobCancellationRequested) return;
  const text = String(data?.text || "").trim();
  if (!text) return;
  livePartialText.textContent = text;
  livePartialTranscript.classList.remove("d-none");
...
function addSegment(data) {
  ...
  const segment = {
    ...
    speakerId: String(data.speaker_id || "").trim() || null,
    ...
    words: Array.isArray(data.words)
      ? data.words
        .map(word => ({
          ...
          speakerId: String(word?.speaker_id || "").trim() || null,
        }))
...
function buildSrt(mode) {
  if (mode === "source") {
    const entries = normalizeSrtTimeline(sortedSourceSegments().map(segment => ({
      ...
      text: withSpeakerLabel(segment, segment.sourceText),
    })));
  ...
  const entries = sortedDisplayCues().map(cue => {
    const speakerId = cue.sourceIds
      .map(sourceId => sourceSegments.get(sourceId)?.speakerId)
      .find(Boolean) || null;
    const translation = withSpeakerLabel(
      { speakerId },
      cue.lines.join("\n"),
    );
  ...
  for (const segment of sortedSourceSegments()) {
    ...
    entries.push({
      ...
      text: mode === "bilingual"
        ? `${withSpeakerLabel(segment, translation)}\n${segment.sourceText}`
        : withSpeakerLabel(segment, translation),
    });
  }
...
function connectTranscriptionJob(job, uploadFilename = "") {
  ...
  eventSource.addEventListener("partial_segment", event => {
    try {
      showLivePartialTranscript(JSON.parse(event.data || "{}"));
    } catch (error) {
      console.error("Could not process partial transcription", error);
    }
  });
...
confirmLanguageBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(await readError(response));
    requestedSourceLanguage = language === "auto" ? "" : language;
    if (language !== "auto") detectedSourceLanguage = language;
    // Keep the main form in sync with the language selected in the confirmation step.
    sourceLanguage.value = language === "auto" ? "" : language;
    languageConfirmation.classList.add("d-none");
    revealResults();
...
showSourceSubtitles.addEventListener("change", () => {
  let currentTime = null;
  try {
    currentTime = youtubePlayerController?.getCurrentTime?.();
  } catch (_) {}
  const segment = Number.isFinite(currentTime)
    ? playbackSegmentAt(currentTime)
    : sourceSegments.get(activePlaybackSegmentId) || null;
  updateVideoSubtitleOverlay(segment, currentTime);
});
...
speakerDiarization.addEventListener("change", () => {
  if (speakerDiarization.checked) ignoreSubtitles.checked = true;
  ignoreSubtitles.disabled = uploadMode || speakerDiarization.checked;
});
