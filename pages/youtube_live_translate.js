...
const translations = {
  "zh-Hant": {
    ...
    sourceLanguage: "原文語言", targetLanguage: "翻譯目標語言", translationType: "翻譯類型", translationTypeStd: "Std（Qwen3 14B）", translationTypePro: "Pro（Gemini，按 token 計費）", transcriptionMode: "轉譯模式", modeAccurate: "高精度（較慢）", modeFast: "快速", autoDetect: "自動偵測", languageEnglish: "英文", languageJapanese: "日文", languageKorean: "韓文", languageThai: "泰文", languageTraditionalChinese: "繁體中文",
    ...
    translationCounts: "已翻譯 {translated} / 已收到 {received} 段 · 等待翻譯 {waiting} 段", translationPercentDone: "翻譯完成 {percent}% · 失敗 {failed} 段", translationErrorCodes: "錯誤碼 {codes}", proUsageSummary: "Pro countTokens {tokens} · 預估 US${usd} / NT${twd}", sameLanguage: "原文與目標語言相同，已略過翻譯。", unsupportedLanguage: "目前翻譯服務不支援偵測到的語言：{language}",
  ...
  en: {
    ...
    sourceLanguage: "Source language", targetLanguage: "Target language", translationType: "Translation tier", translationTypeStd: "Std (Qwen3 14B)", translationTypePro: "Pro (Gemini, per-token billing)", transcriptionMode: "Transcription mode", modeAccurate: "High accuracy (slower)", modeFast: "Fast", autoDetect: "Auto detect", languageEnglish: "English", languageJapanese: "Japanese", languageKorean: "Korean", languageThai: "Thai", languageTraditionalChinese: "Traditional Chinese",
    ...
    translationCounts: "Translated {translated} / {received} received · {waiting} waiting", translationPercentDone: "Translation {percent}% · {failed} failed", translationErrorCodes: "Error code {codes}", proUsageSummary: "Pro countTokens {tokens} · est. US${usd} / NT${twd}", sameLanguage: "Source and target languages match. Translation was skipped.", unsupportedLanguage: "The translation service does not support the detected language: {language}",
  ...
  ja: {
    ...
    translationCounts: "翻訳済み {translated} / 受信 {received} 件・待機 {waiting} 件", translationPercentDone: "翻訳 {percent}%・失敗 {failed} 件", translationErrorCodes: "エラーコード {codes}", proUsageSummary: "Pro countTokens {tokens}・推定 US${usd} / NT${twd}", sameLanguage: "原文と翻訳先が同じため、翻訳を省略しました。", unsupportedLanguage: "検出された言語は現在サポートされていません：{language}",
  ...
  ko: {
    ...
    sourceLanguage: "원문 언어", targetLanguage: "번역 언어", translationType: "번역 유형", translationTypeStd: "Std(Qwen3 14B)", translationTypePro: "Pro(Gemini, 토큰 과금)", transcriptionMode: "전사 모드", modeAccurate: "고정밀(느림)", modeFast: "빠름", autoDetect: "자동 감지", languageEnglish: "영어", languageJapanese: "일본어", languageKorean: "한국어", languageThai: "태국어", languageTraditionalChinese: "번체 중국어",
    ...
    translationCounts: "번역 {translated} / 수신 {received}개 · 대기 {waiting}개", translationPercentDone: "번역 {percent}% · 실패 {failed}개", translationErrorCodes: "오류 코드 {codes}", proUsageSummary: "Pro countTokens {tokens} · 예상 US${usd} / NT${twd}", sameLanguage: "원문과 대상 언어가 같아 번역을 건너뛰었습니다.", unsupportedLanguage: "감지된 언어는 현재 지원되지 않습니다: {language}",
...
let selectedTranslationType = "std";
...
function recordTranslationUsage(usage) {
  if (selectedTranslationType !== "pro" || !usage || typeof usage !== "object") return;
  addTranslationUsage(translationUsageTotals, usage);
  updateTranslationProgress();
}

function withTranslationUsage(message) {
  if (selectedTranslationType !== "pro") return message;
  const summary = t("proUsageSummary", {
...
async function beginTranscription() {
  ...
  selectedTranslationType = translationType.value === "pro" ? "pro" : "std";
