...
from media_audio_stream import MediaAudioSource, decode_media_prefix
...
YOUTUBE_PO_TOKEN = (
    os.getenv("YOUTUBE_PO_TOKEN")
    or os.getenv("PO_TOKEN_VALUE")
    or ""
).strip()
# bgutil provider 會依影片自動建立 PO Token。空字串代表停用；避免在未安裝
# provider 的既有環境中改變 yt-dlp 行為。
YOUTUBE_POT_PROVIDER_URL = os.getenv("YOUTUBE_POT_PROVIDER_URL", "").strip().rstrip("/")


def youtube_javascript_runtime_options() -> dict[str, Any]:
    """Use Node and yt-dlp's maintained EJS component for current player JS.

    This mirrors YouTubeSimpleDownloader's successful desktop configuration.
    The component is fetched and verified by yt-dlp; if it is unavailable,
    yt-dlp retains its normal built-in fallback behaviour.
    """
    return {
        "js_runtimes": {"node": {}},
...
def youtube_player_client_attempts() -> list[Optional[list[str]]]:
    """Provider 可用時優先使用已驗證可下載的 GVS PO Token client。"""
    if not YOUTUBE_POT_PROVIDER_URL:
        return YOUTUBE_PLAYER_CLIENT_ATTEMPTS
    return [
        # Some current mweb GVS URLs are still rejected on Windows even when
        # they contain a token.  tv_simply is generated by bgutil and has proven
        # to work with yt-dlp's native downloader for this service.
        ["tv_simply", "default"],
        *[
            clients
            for clients in YOUTUBE_PLAYER_CLIENT_ATTEMPTS
            if clients not in (["tv_simply", "default"],)
        ],
    ]
...
def youtube_extractor_args(
    ...
    """建立 yt-dlp YouTube extractor args，不記錄敏感的 PO Token。"""
...
def get_youtube_video_info(url: str, cookies_file: Path) -> dict[str, Any]:
    ...
    options = {
        ...
        "quiet": True,
        "no_warnings": True,
        **youtube_javascript_runtime_options(),
    }
...
def download_youtube_audio(url: str, output_dir: str, cookies_file: Path):
    ...
    def build_options(player_clients: Optional[list[str]], attempt_dir: str):
        ...
        extractor_args = youtube_extractor_args(player_clients)
        if extractor_args:
            options["extractor_args"] = extractor_args
        if cookies_file.exists():
            options["cookiefile"] = str(cookies_file)
...
def get_youtube_audio_stream_source(
    ...
    errors: list[str] = []
    cookie_modes = [False, True] if cookies_file.exists() else [False]
        ...
        for player_clients in youtube_player_client_attempts():
            for use_cookies in cookie_modes:
                ...
                try:
                        ...
                        if not stream_url:
                            ...
                            for candidate in candidates:
                                if (
                                    ...
                                    stream_url = str(candidate["url"]).strip()
                                    break
                        if not stream_url:
                        ...
                        headers.update(
                            {
                                str(name): str(value)
                                for name, value in (
                                    selected_format.get("http_headers") or {}
                                ).items()
                                if value is not None
                            }
                        )
                    ...
                    decode_media_prefix(source, 1.0)
                    return source, info
                except Exception as exc:
                    cookie_mode = "cookies" if use_cookies else "anonymous"
                    logger.warning(
                        "YouTube audio stream candidate failed validation: url=%s "
                        "player_clients=%s auth=%s error=%s",
                        attempt_url,
                    ...
                    errors.append(
                        f"{attempt_url} / {player_clients or 'default'} / "
                        f"{cookie_mode}: {str(exc)}"
                    )
